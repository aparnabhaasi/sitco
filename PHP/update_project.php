...
if ($method === 'POST' && isset($_POST['id']) && isset($_POST['title'])) {
    ...
    if (isset($_FILES['file'])) {
        ...
        if (move_uploaded_file($file_temp, $destination)) {
            ...
            $stmt = $conn->prepare("UPDATE about_project SET title = ?, image = ? WHERE id = ?");
        ...
        $stmt = $conn->prepare("UPDATE about_project SET title = ? WHERE id = ?");
    ...
    $stmt = $conn->prepare("SELECT image FROM about_project WHERE id = ?");
    ...
    if ($stmt->num_rows === 1) {
        ...
        $imagePath = $_SERVER['DOCUMENT_ROOT'] . '/sitco/public/Images/project/' . $image;
        ...
        $stmt->close();
        $deleteStmt = $conn->prepare("DELETE FROM about_project WHERE id = ?");
        $deleteStmt->bind_param("i", $id);

        if ($deleteStmt->execute()) {
            echo json_encode(["success" => "Image deleted successfully"]);
        } else {
            echo json_encode(["error" => "Failed to delete image: " . $deleteStmt->error]);
        }
