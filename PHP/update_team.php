...
if ($method === 'POST' && isset($_POST['id']) && isset($_POST['tname'])&& isset($_POST['des'])&& isset($_POST['description'])) {
    $id = $_POST['id'];
    $tname = $_POST['tname'];
    $des = $_POST['des'];
    ...
    if (isset($_FILES['file'])) {
        ...
        if (move_uploaded_file($file_temp, $destination)) {
            ...
            $stmt = $conn->prepare("UPDATE team SET  image = ?, name = ?, designation = ?, description = ? WHERE id = ?");
            $stmt->bind_param("ssssi", $file, $tname, $des, $description, $id);
...
elseif ($method === 'DELETE' && isset($_GET['id'])) {
    ...
    if ($stmt->num_rows === 1) {
        ...
        $imagePath = $_SERVER['DOCUMENT_ROOT'] . '/sitco-master/public/Images/team/' . $image;
